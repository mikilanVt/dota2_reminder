...
console.log(`Core build files gzip: ${totalGzipBytes - itemShopGzipBytes} B / limit ${TOTAL_GZIP_LIMIT} B`);
console.log(`Item shop assets (${itemShopFiles} files, loaded on demand): ${itemShopGzipBytes} B / limit ${ITEM_SHOP_GZIP_LIMIT} B`);
console.log(`All build files gzip total: ${totalGzipBytes} B / limit ${TOTAL_GZIP_LIMIT + ITEM_SHOP_GZIP_LIMIT} B`);

if (jsGzipBytes > JS_GZIP_LIMIT)
  throw new Error('Initial JavaScript budget exceeded.');
if (totalGzipBytes - itemShopGzipBytes > TOTAL_GZIP_LIMIT)
